...
export const createRoleStaffService = async (formData: any) => {
    try {
        ...
        const roleStaffModel = mapToRoleStaffModel(rawData.roleStaff);
        const permissions: RolePermissionDto[] = Array.isArray(rawData.roleStaff.permissions)
            ? rawData.roleStaff.permissions.map((p: any) => mapToRolePermissionModel(p))
            : []
