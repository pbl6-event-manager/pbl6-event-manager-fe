...
type TeamTab = "staffs" | "roles"

export default function TeamManagementPage() {
    const navigate = useNavigate()
    const [searchParams] = useSearchParams()
    const tab = (searchParams.get("tab") || "staffs") as TeamTab

    const handleTabChange = (newTab: TeamTab) => {
        navigate(`?tab=${newTab}`)
    }
    // const handleTabChange = (newTab: TeamTab) => {
    //     navigate(`/organizer/settings/${newTab === "users" ? "members/staffs" : "members/roles"}`)
    // }
    return (
